 ...
 *   - wp_global_lot_item_shop_config (min_qty, max_qty)
 *   - wp_global_reorder_suggestion (đề xuất mua)
 */
...
class TGS_Collector_Warehouse_MinMax extends TGS_Collector_Base
{
    public static function collect($date_from, $date_to)
    {
        ...
        $reorder_table = 'wp_global_reorder_suggestion';
        ...
        $has_reorder = ($wpdb->get_var("SHOW TABLES LIKE '{$reorder_table}'") === $reorder_table);

        // ── 1) Dưới MIN ──
        if ($has_config && $has_inv) {
            $below = $wpdb->get_results($wpdb->prepare(
                "SELECT
                    inv.blog_id, inv.sku, inv.closing_qty,
                    cfg.min_qty, cfg.max_qty,
                    (cfg.min_qty - inv.closing_qty) as shortage
                 FROM {$inv_table} inv
                 ...
                 WHERE inv.rollup_date = %s
                   AND cfg.is_active = 1
                   AND cfg.min_qty > 0
                   AND inv.closing_qty < cfg.min_qty
                 ORDER BY (cfg.min_qty - inv.closing_qty) DESC
                 LIMIT 300",
                $date_to
            ));
        ...
        // ── 2) Vượt MAX ──
        if ($has_config && $has_inv) {
            $above = $wpdb->get_results($wpdb->prepare(
                "SELECT
                    inv.blog_id, inv.sku, inv.closing_qty,
                    cfg.max_qty,
                    (inv.closing_qty - cfg.max_qty) as surplus
                 FROM {$inv_table} inv
                 INNER JOIN {$config_table} cfg
                    ON inv.blog_id = cfg.blog_id AND inv.sku = cfg.product_sku
                 WHERE inv.rollup_date = %s
        ...
        if ($has_inv) {
            $near = $wpdb->get_results($wpdb->prepare(
                "SELECT blog_id, sku, exp_date, near_expiry_qty, closing_qty
                 FROM {$inv_table}
                 WHERE rollup_date = %s
                   AND near_expiry_qty > 0
                 ORDER BY near_expiry_qty DESC
                 LIMIT 200",
                $date_to
            ));
