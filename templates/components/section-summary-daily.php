...
<div class="section">
        ...
        <tr>
            ...
            <td width="50%" style="padding:0 0 12px 6px; vertical-align:top;">
                <div style="background:#f8fafc; border-radius:8px; padding:14px 16px; border-left:4px solid #28a745;">
                    <div style="font-size:11px; color:#6c757d; text-transform:uppercase;">Đã Thu Bán Hàng</div>
                    <div style="font-size:20px; font-weight:700; color:#28a745;"><?php echo $fmt($daily['total_net'] ?? 0); ?>đ</div>
                    <div style="font-size:11px; color:#888;">Tiền hàng trước CK: <?php echo $fmt($daily['total_gross'] ?? 0); ?>đ</div>
                </div>
            </td>
        </tr>
        <tr>
    ...
    <div style="margin-top:8px;">
        ...
        <?php
            $top_total_orders = 0;
            $top_total_net = 0;
            foreach ($summary['top_shops'] as $top_shop) {
                $top_total_orders += (int) ($top_shop['order_count'] ?? 0);
                $top_total_net += (float) ($top_shop['net'] ?? 0);
            }
        ?>
        <table class="data-table" style="width:100%; border-collapse:collapse; font-size:13px; margin-top:8px;">
            <tr>
                ...
                <th style="background:#f0f4f8; color:#1e3a5f; padding:8px; text-align:right; border-bottom:2px solid #dee2e6;">Số đơn</th>
            ...
            <tr>
                ...
                <td style="padding:8px; border-bottom:1px solid #f0f0f0; text-align:right;"><?php echo (int) ($t['order_count'] ?? 0); ?></td>
            ...
            <?php endforeach; ?>
            <tr>
                <td colspan="2" style="padding:8px; background:#f0f4f8; font-weight:700; border-top:2px solid #dee2e6;">TỔNG TOP <?php echo count($summary['top_shops']); ?></td>
                <td style="padding:8px; background:#f0f4f8; font-weight:700; border-top:2px solid #dee2e6; text-align:right;"><?php echo $fmt($top_total_orders); ?></td>
                <td style="padding:8px; background:#f0f4f8; font-weight:700; border-top:2px solid #dee2e6; text-align:right; color:#28a745;"><?php echo $fmt($top_total_net); ?>đ</td>
            </tr>
        </table>
    </div>
    <?php endif; ?>
